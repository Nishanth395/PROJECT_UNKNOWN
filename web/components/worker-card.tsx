...
import { Star, ShieldCheck, MapPin, Briefcase, Check, CalendarPlus } from "lucide-react";

interface WorkerCardProps {
  worker: MatchedWorker;
  rank?: number;
  onRequestBooking?: (worker: MatchedWorker) => void;
}

export function WorkerCard({ worker, rank, onRequestBooking }: WorkerCardProps) {
  return (
    <div
      data-testid="worker-card"
      className="relative rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:shadow-md hover:border-slate-300 space-y-4"
    >
      {/* Optional Rank Badge */}
      {rank && (
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6">
        ...
        {/* Match Score & Booking Action */}
        <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center border-t sm:border-t-0 pt-4 sm:pt-0 border-slate-100 sm:min-w-[150px] space-y-0 sm:space-y-3">
          ...
          {onRequestBooking && (
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold py-2.5 px-4 shadow-sm transition"
            >
