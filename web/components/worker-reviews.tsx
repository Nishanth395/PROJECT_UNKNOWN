...
import { WorkerReviewsResponse, Review } from "@/types/review";
...
export function WorkerReviews({
  ...
  const [reviewsData, setReviewsData] = useState<WorkerReviewsResponse | null>(null);
  ...
  const loadReviews = useCallback(async () => {
    ...
    try {
      ...
      if (err instanceof ApiException) {
        ...
        setError("Failed to load worker reviews. Please check your connection.");
      }
    }
  }, [workerId]);
  ...
  if (isLoading) {
    return (
      <div
        data-testid="worker-reviews-loading"
        className={`rounded-3xl border border-slate-200 bg-white p-6 sm:p-7 shadow-sm text-center ${className}`}
      >
  ...
  if (error) {
    return (
      <div className={className}>
        <ErrorAlert message={error} onRetry={loadReviews} />
      </div>
    );
  }
  ...
  return (
    <div
      data-testid="worker-reviews"
      className={`rounded-3xl border border-slate-200 bg-white p-6 sm:p-7 shadow-sm space-y-6 ${className}`}
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
        ...
        {reviewsData && (
          <div
            data-testid="worker-reputation-summary"
            className="flex items-center gap-3 bg-slate-50 border border-slate-200/80 px-4 py-2 rounded-2xl"
          >
            <div className="flex items-center gap-1.5 font-black text-slate-900 text-lg">
              <Star className="h-5 w-5 fill-amber-400 text-amber-400" />
              <span>
                {reviewsData.average_rating !== null && reviewsData.average_rating !== undefined && reviewsData.average_rating > 0
      ...
      {hasReviews ? (
        <div className="space-y-4">
          {reviewsData.items.map((review: Review) => (
            <div
              key={review.id}
              data-testid="worker-review-item"
        ...
        /* Empty Reviews State */
        <div
          data-testid="worker-reviews-empty"
          className="rounded-2xl bg-slate-50 border border-slate-100 p-8 text-center space-y-2"
        >
