...
import { CalendarCheck, ArrowLeft, RefreshCw, PlusCircle } from "lucide-react";
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          ...
          <p className="text-sm text-slate-600">
            Track engagement statuses with verified trade professionals and manage appointments.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            href="/request/new"
            className="inline-flex items-center gap-1.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold px-4 py-2.5 text-xs shadow-sm transition"
          >
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {bookings.items.map((b) => (
              <CustomerBookingCard key={b.booking_id} booking={b} />
            ))}
          </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-12 text-center space-y-4 shadow-sm">
          ...
          <div className="space-y-1 max-w-sm mx-auto">
            ...
            <p className="text-xs text-slate-500">
              {activeTab === "pending"
                ? "You have no bookings currently pending confirmation."
                : "When you request verified workers from your matched results, bookings will appear here."}
            </p>
          </div>
          <Link
            href="/requests"
          ...
          >
            <span>View My Service Requests</span>
          </Link>
        </div>
      )}
    </div>
...
export default function CustomerBookingsPage() {
  return (
    <Suspense fallback={<LoadingSpinner message="Loading your bookings..." />}>
